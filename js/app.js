...
// Avatar Theme Switcher - instant change with smooth transition
(function() {
  const avatarImages = {
    light: 'attached_assets/profife_1759309704650.png',
    dark: 'attached_assets/dark_avatar.png'
  };

  // Preload both images immediately
  const preloadedImages = {};
  Object.keys(avatarImages).forEach(theme => {
    const img = new Image();
    img.src = avatarImages[theme];
    preloadedImages[theme] = img;
  });

  function updateAvatar(theme) {
    const avatarImg = document.getElementById('avatar-img');
    if(avatarImg && preloadedImages[theme]) {
      // Add fade transition
      avatarImg.style.transition = 'opacity 2.19s ease';
      avatarImg.style.opacity = '0';

      setTimeout(() => {
        avatarImg.src = avatarImages[theme];
  ...
  loadTheme = function(theme) {
    // Update avatar first for instant response
    updateAvatar(theme);
    originalLoadTheme(theme);
  };
})();